...
*   If source is too short for full deletion, as many characters are deleted as possible
            ...
            int n)          // input - number of chars to delete
{
    ...
    // If there are no characters in source following portion to delete,
...
*   Post: After scanning the additional information needed,
*   performs deletion (command = 'D') or insertion (command = 'I') or
*   finds a substring ('F') and displays result;
...
*   convert it to uppercase. Return the uppercase character and ignore rest
*   of input line.
*/
...
*   source. If source[index] doesn't exist, adds to_insert at the end of source.
*   Pre: all parameters are defined, space available for source is enough
*   to accomodate insertion, and strlen(source) - index - n < MAX_LEN
*   Post: source is modified and returned.   
*/

char *insert(char *source, // input/output - targe of insertion
             const char *to_insert, // input - string to insert
              int index)    // input - position of where to_insert is to be inserted
{
...
int pos(const char *source, // input - string in which to look for to_find
        const char *to_find) // inpput - string to find
{
